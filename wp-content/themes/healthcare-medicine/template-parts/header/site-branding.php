...
<div class="headerbox py-3 <?php if( is_user_logged_in() && !isset( $wp_customize ) ){ echo "login-user";} ?>" data-sticky="<?php echo esc_attr($data_sticky); ?>">
  <div class="container">
    <div class="row">
      <div class="col-lg-3 col-md-4 col-9 align-self-center">
        ...
        if($online_pharmacy_logo_settings == 'Different Line'){ ?>
          <div class="logo mb-md-0">
            ...
            <?php if(get_theme_mod('online_pharmacy_site_title_text',true) != ''){ ?>
              <h1><a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?></a></h1>
            <?php }?>
            <?php $healthcare_medicine_description = get_bloginfo( 'description', 'display' );
            if ( $healthcare_medicine_description || is_customize_preview() ) : ?>
              <?php if(get_theme_mod('online_pharmacy_site_tagline_text',false) != ''){ ?>
                <p class="site-description mb-0"><?php echo esc_html($healthcare_medicine_description); ?></p>
              <?php }?>
            <?php endif; ?>
          </div>
